...
public class MinimumSubsetSumDifference {
	...
	public int minDiffernce(int arr[], int n) {
		...
		for(int i=mid;i<=sum;i++) {
			if(hasSubsetSum(i,arr,n)) {
				minDifference=Math.abs(i-(sum-i));
				break;
			}
		}
	...
	private boolean hasSubsetSum(int target, int [] arr, int n) {
	...
	public static void main(String[] args) {
		...
		int [] arr= {1,2,7};
		System.out.println(new MinimumSubsetSumDifference().minDiffernce(arr, 3));
