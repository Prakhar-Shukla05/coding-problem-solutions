...
/*
 * This application makes use of Held-Karp Algorithm to solve the Traveling salesman Problem in exponential time which although bad is far better than factorial time brute force approach.
 * The adjacency matrix is given by costMatrix array. It is an asymmetric graph meaning cost of going from A to B is not the same as going from B to A.
 * Time complexity- O(2^n * n^2)
 */
public class TravelingSalesmanProblemUsingHeldKarp {
	...
	public int minCost(int[][] costMatrix) {
		...
		//{},{1},{2],{3},{1,2},{2,3},{1,3}
		List<Set<Integer>> allSets = generateCombination(costMatrix.length - 1);

		for (Set<Integer> set : allSets) {
			for (int currentVertex = 1; currentVertex < costMatrix.length; currentVertex++) {
				...
				for (int prevVertex : set) {
					int cost = costMatrix[prevVertex][currentVertex] + getCost(copySet, prevVertex, minCostDP);
					if (cost < minCost) {
						minCost = cost;
				...
				if (set.size() == 0) {
					minCost = costMatrix[0][currentVertex];
				}
		...
		// iterating
		Set<Integer> copySet = new HashSet<>(set);
		// Cost to reach 0 traveling through {1,2,3}
		for (int k : set) {
			int cost = costMatrix[k][0] + getCost(copySet, k, minCostDP);
			if (cost < min) {
				min = cost;
				prevVertex = k;
			}
		...
		printTour(parent, costMatrix.length);
		System.out.print("The minimum cost of the tour is - ");
		return min;
	}
	...
	private void printTour(Map<Index, Integer> parent, int totalVertices) {
		...
		System.out.println("TSP Tour or Minimum Cost Hamiltonian Circuit is-");
	...
	public static void main(String[] args) {
		...
		int[][] costMatrix = { { 0, 1, 15, 6 }, { 2, 0, 7, 3 }, { 9, 6, 0, 12 }, { 10, 4, 8, 0 } };
		System.out.println(new TravelingSalesmanProblemUsingHeldKarp().minCost(costMatrix));
	}
